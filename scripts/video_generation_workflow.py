...
def main():
    ...
    parser.add_argument('--initial-frame-prompt', type=str, default="In the style of pop art, a magestic scene unfolds")
    ...
    seed_parent = os.path.join(r"D:\utils\ComfyUI_windows_portable\ComfyUI\output", str(args.id))
    seed_path =  os.path.join(seed_parent, "seed.png")
    os.makedirs(seed_parent, exist_ok=True)
    ...
    if args.part_index == 0:
        try:
            ...
            prompt = args.initial_frame_prompt
            ...
            print("Error generating seed image: ", e)
            return -1

    while not os.path.exists(seed_path): # "spin lock"
        print("Seed image not created, sleeping for 5 seconds")
        time.sleep(5)
    ...
    with torch.inference_mode():
        ...
        mochivaeencoderloader = NODE_CLASS_MAPPINGS["MochiVAEEncoderLoader"]()
        mochivaeencoderloader_3 = mochivaeencoderloader.loadmodel(
            model_name="mochi_preview_vae_encoder_bf16_.safetensors", precision="bf16"
        )

        loadimage = NODE_CLASS_MAPPINGS["LoadImage"]()
        loadimage_4 = loadimage.load_image(image=seed_path)

        mochiimageencode = NODE_CLASS_MAPPINGS["MochiImageEncode"]()
        mochiimageencode_1 = mochiimageencode.encode(
            ...
            min_block_size=1,
            normalize=True,
            encoder=get_value_at_index(mochivaeencoderloader_3, 0),
            images=get_value_at_index(loadimage_4, 0),
        ...
        emptymochilatentvideo_21 = emptymochilatentvideo.generate(
            width=480, height=480, length=args.frames, batch_size=1
        )
        ...
        mochimodelloader = NODE_CLASS_MAPPINGS["MochiModelLoader"]()
        mochimodelloader_5 = mochimodelloader.loadmodel(
            model_name="mochi\mochi_preview_dit_fp8_e4m3fn.safetensors",
        ...
        mochisigmaschedule = NODE_CLASS_MAPPINGS["MochiSigmaSchedule"]()

        mochisigmaschedule_15 = mochisigmaschedule.loadmodel(
            num_steps=args.steps, threshold_noise=0.025, linear_steps=15, denoise=0.6
        )
        ...
        for q in range(1):
            ...
            mochisampler_2 = mochisampler.process(
                width=768,
                height=768,
                ...
                cfg=7,
                seed=6335316662,
                model=get_value_at_index(mochimodelloader_5, 0),
                positive=get_value_at_index(cliptextencode_6, 0),
                negative=get_value_at_index(cliptextencode_7, 0),
                opt_sigmas=get_value_at_index(mochisigmaschedule_15, 0),
                samples=get_value_at_index(mochiimageencode_1, 0),
            )
            ...
            mochidecode_55 = mochidecode.decode(
                ...
                samples=get_value_at_index(mochisampler_2, 0),
            )
